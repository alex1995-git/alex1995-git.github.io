...
begin
	...
	UPDATE tabonados_masive tms SET 
		mensaje = 'El campo monto no tiene valor'
...
delete from tabono;
	...
	asiento = tm.asiento, localidad = tm.abre_localidad, sublocalidad = tm.sub_localidad
	from tabonados_masive tm 
		where
			tm.asiento  =  t.asiento AND t.estado in ('A','O')
	...
	(asiento, columna, estado, fila, localidad, precio, sublocalidad, tipo_area, zona)
	(select	
			tm.asiento ,
			null,
			'A',
		...
		where
			code_unico = pi_secuencial
			and tm.asiento  not in (select asiento from tasiento q where q.estado in ('A','O'))
			and(tm.asiento !=''or tm.asiento !=null)
			AND coalesce(tm.mensaje,'')=''
	) ;
...
----Valida si existe alguna observacion
	...
	WHERE l.secuencial = pi_secuencial
